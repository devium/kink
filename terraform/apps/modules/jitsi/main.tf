locals {
  ...
  keycloak_adapter_url = "https://raw.githubusercontent.com/nordeck/jitsi-keycloak-adapter/${var.config.version_keycloak_adapter}/templates"
...
resource "kubernetes_config_map_v1" "web_config" {
  ...
  data = {
    ...
    "05-keycloak-adapter" = <<-BASH
      #!/usr/bin/env bash
      wget ${local.keycloak_adapter_url}/usr/share/jitsi-meet/body.html -O /usr/share/jitsi-meet/body.html
      wget ${local.keycloak_adapter_url}/usr/share/jitsi-meet/static/oidc-adapter.html -O /usr/share/jitsi-meet/static/oidc-adapter.html
      wget ${local.keycloak_adapter_url}/usr/share/jitsi-meet/static/oidc-redirect.html -O /usr/share/jitsi-meet/static/oidc-redirect.html
      wget ${local.keycloak_adapter_url}/jitsi-web-container/defaults/meet.conf -O /defaults/meet.conf
    BASH
  }
}
  ...
  values = [<<-YAML
    ...
    web:
      ...
      extraVolumeMounts:
        ...
        - name: web-config
          mountPath: /etc/cont-init.d/05-keycloak-adapter
          subPath: 05-keycloak-adapter
